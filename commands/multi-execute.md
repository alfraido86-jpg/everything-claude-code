...
- `{{GEMINI_MODEL_FLAG}}`: When using `--backend gemini`, replace with `--gemini-model gemini-3-pro-preview` + a trailing space (note: append a space after the flag); use empty string for codex
