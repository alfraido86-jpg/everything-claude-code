...
function Invoke-BackupPhase {
    ...
    foreach ($item in $itemsToQuarantine) {
        if (Test-Path $item) {
            if (Test-PathLocked $item) {
                throw "Cannot quarantine locked path: $item. Please close applications and retry."
            }
            
            New-Item -ItemType Directory -Path $quarantineDir -Force | Out-Null
            $itemName = Split-Path -Leaf $item
            $destPath = Join-Path $quarantineDir $itemName
            
            Write-Status "Moving to quarantine: $item"
